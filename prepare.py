...
def fit_and_scale(scaler, train, validate, test):
    ...
    # train, validate & test
    scaler.fit(train[orig])
    scaled_train = pd.DataFrame(data=scaler.transform(train[orig]), columns=orig,)
    scaled_validate = pd.DataFrame(data=scaler.transform(validate[orig]), columns=orig)
    scaled_test = pd.DataFrame(data=scaler.transform(test[orig]), columns=orig)
